...
@Getter
public class HomeWorkSupportViewModel extends BaseCommunityViewModel {
    private final MutableLiveData<Boolean> _isLoading = new MutableLiveData<>(false);
    private final LiveData<Boolean> isLoading = _isLoading;
    private boolean hasNextPage = true;
    private int currentPage = 1;

    public HomeWorkSupportViewModel(@NonNull Application application) {
        super(application);
    }
    ...
    public void loadingPage(TagType tagType, Context context, ResultCallback<LinkedHashSet<PostResponse>> callback){
        ...
        postRepository.getPostByTagType(tagType,currentPage, SystemConstant.LIMIT,context,new PostRepository.GetPostCallBack() {
            ...
            public void onSuccess(LinkedHashSet<PostResponse> postResponses)  {
                ...
                if(callback != null){
                    callback.onSuccess(currentPosts);
                }
